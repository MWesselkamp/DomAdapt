...
def nn_selection(X, Y, hp_list, splits, searchsize):
    ...
    visualizations.plot_nn_loss(running_losses["rmse_train"], running_losses["rmse_val"], figure = "", hparams = hparams, model="mlp")
    
    return(running_losses, y_tests, y_preds)
    
#%% Random Grid search: Paralellized
    
def nn_selection_parallel(X, Y, hp_list, splits, searchsize, q, hp_search = []):
    ...
    visualizations.plot_nn_loss(running_losses["rmse_train"], running_losses["rmse_val"], figure = searchsize, hparams = hparams, model="mlp")
    visualizations.plot_nn_predictions(y_tests_nn, y_preds_nn, history = hparams["history"], figure = searchsize, model="mlp")
...
def train_model(X, Y, nnp):
    ...
    # If hisotry > 0, batches are extended by the specified lags.
            x, y = preprocessing.create_batches(X_train, y_train, hparams["batchsize"], hparams["history"])
