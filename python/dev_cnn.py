...
def train_model_CV(hparams, model_design, X, Y, splits, eval_set, data_dir,
    ...
    for train_index, test_index in kf.split(X):
        ...
        if finetuning:
            ...
            #model.load_state_dict(torch.load(os.path.join(data_dir, f"model{split}.pth")))
...
def conv_selection_parallel(X, Y, hp_list, epochs, splits, searchsize, datadir, q, hp_search = []):
    ...
    start = time.time()
    running_losses,performance, y_tests_nn, y_preds_nn = train_model_CV(hparams, model_design, X, Y, splits)
    end = time.time()
...
def selected(X, Y, model_params, epochs, splits, 
    ...
    if not data_dir is None:
        data_dir = os.path.join(data_dir, f"cnn")
