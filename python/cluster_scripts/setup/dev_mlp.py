...
#%%
def train_model_CV(hparams, model_design, X, Y, eval_set, dropout_prob, dropout,
    ...
    for train_index, test_index in kf.split(X):
        ...
        if featuresize is None:
            ...
            model = models.MLPmod(featuresize, model_design["dimensions"], model_design["activation"], dropout_prob, dropout)
...
def selected(X, Y, model,typ, best_model, epochs,change_architecture = False,
    ...
    if best_model["featuresize"] is None:
    ...
    elif not traindata_perc is None:
    ...
    if not traindata_perc is None:
        data_dir = os.path.join(data_dir, f"sigmoid/data{traindata_perc}perc")
