...
#%%
def set_model_parameters(model, typ, sparse, epochs, change_architecture, adaptive_pooling,
                         X, Y, featuresize = None, data_dir = r"/home/fr/fr_fr/fr_mw263"):
    
    results = pd.read_csv(os.path.join(data_dir, f"output/grid_search/grid_search_results_mlp2.csv"))
...
def train_network(model,typ, sparse, site, epochs, q, adaptive_pooling = False, dropout_prob = 0.0, change_architecture = False, 
    ...
    if sparse == 0:
      ind = np.random.choice(X.shape[0], 50)
      X, Y = X[ind], Y[ind]
      
    eval_set = {"X_test":X_test, "Y_test":Y_test}
        
    hparams, model_design = set_model_parameters(model, typ, sparse, epochs, change_architecture, adaptive_pooling, X, Y)
   
    start = time.time()
    
    data_dir = os.path.join(data_dir, f"output/sparse/models/{model}{typ}/sparse{sparse}")
