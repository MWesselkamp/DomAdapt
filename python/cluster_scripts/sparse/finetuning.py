...
#%%
def training_CV(hparams, model_design, X, Y, pretrained_type, sparse, feature_extraction, eval_set, data_dir,
                   save, splits = 5):
    ...
    for train_index, test_index in kf.split(X):
        ...
        if save:
            if not feature_extraction is None:
                torch.save(model.state_dict(), os.path.join(data_dir, f"sparse{sparse}/setting1/model{i}.pth"))
            else:
                torch.save(model.state_dict(), os.path.join(data_dir, f"sparse{sparse}/setting0/model{i}.pth"))
...
#%%
def finetune(X, Y, epochs, model, pretrained_type, sparse, 
             feature_extraction = None, eval_set = None, save=True, 
             data_dir = r"/home/fr/fr_fr/fr_mw263"):
    
    gridsearch_results = pd.read_csv(os.path.join(data_dir, f"output/grid_search/simulations/grid_search_results_{model}2_adaptPool.csv"))
    ...
    data_dir = os.path.join(data_dir, f"output/sparse/models/{model}{pretrained_type}")

    running_losses,performance, y_tests, y_preds = training_CV(hparams, model_design, X, Y, pretrained_type, sparse, feature_extraction, eval_set, 
                                                               data_dir , 
                                                               save)
    ...
    if not feature_extraction is None:
        data_dir = os.path.join(data_dir, "setting1")
    else:
        data_dir = os.path.join(data_dir, "setting0")
